            ...
            try {
                ...
                if (destination) {
                    try {
                        ...
                        const hotelRes = await fetch(`http://https://triplens-duml.onrender.com/api/hotels/${formattedDestination}`);
                        if (hotelRes.ok) {
                    ...
                    // Fetch Tourist Spots
                    try {
                        const spotRes = await fetch(`http://https://triplens-duml.onrender.com/api/spots/nearby?location=${destination}`);
                        if (spotRes.ok) {
                    ...
                    // Fetch Festivals
                    try {
                        const festRes = await fetch(`http://https://triplens-duml.onrender.com/api/festivals?city=${destination}`);
                        if (festRes.ok) {
                ...
                if (startLocation && destination) {
                    // Fetch Trains
                    try {
                        const trainRes = await fetch(`http://https://triplens-duml.onrender.com/api/trains/search?origin=${startLocation}&destination=${destination}`, {
                            method: 'POST'
                        });
                        if (trainRes.ok) {
                    ...
                    // Fetch Flights
                    try {
                        const flightRes = await fetch(`http://https://triplens-duml.onrender.com/api/flights/search?from=${startLocation}&to=${destination}&date=${date || ''}`);
                        if (flightRes.ok) {
    ...
    const handleSaveItinerary = async () => {
        ...
        try {
            ...
            if (initialItinerary && initialItinerary.id) {
                ...
                res = await fetch(`http://https://triplens-duml.onrender.com/api/itineraries/updateItinerary?itineraryId=${initialItinerary.id}`, {
                    method: 'POST',
                ...
                res = await fetch(`http://https://triplens-duml.onrender.com/api/itineraries/addItinerary?tripId=${activeTripId}`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload)
                });
